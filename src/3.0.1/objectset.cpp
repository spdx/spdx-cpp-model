...
SHACLObjectSet::ptr SHACLObjectSet::findById(std::string const &id,
                                             SHACLObjectSet::ptr const &dflt) const {
...
std::set<SHACLObjectSet::ptr> const &SHACLObjectSet::findByType(std::string const &id) const {
...
void SHACLObjectSet::encode(ListEncoder &encoder, ErrorHandler &errorHandler) const {
  ...
  objects.sort(
      [](std::shared_ptr<SHACLObject> const &first, std::shared_ptr<SHACLObject> const &second) {
  ...
  path.foreachItem(objects, [&](auto &o) {
    ...
    encoder.writeItem(
        [&](ValueEncoder &value_encoder) { o->encode(value_encoder, errorHandler, path); });
    i++;
  });
}

void SHACLObjectSet::decode(ListDecoder &decoder, ErrorHandler &errorHandler, ObjectPath &path) {
  createIndex();

  path.foreachListDecoder(decoder, [&](ValueDecoder &item_decoder) {
    bool ret = item_decoder.readObject(
        [&](ObjectDecoder &obj_decoder) { decodeObject(obj_decoder, errorHandler, path); });
    if (!ret) {
...
void SHACLObjectSet::decode(ObjectDecoder &decoder, ErrorHandler &errorHandler, ObjectPath &path) {
  createIndex();
  decodeObject(decoder, errorHandler, path);
  internalLink();
}

void SHACLObjectSet::decodeObject(ObjectDecoder &decoder, ErrorHandler &errorHandler,
                                  ObjectPath &path) {
...
void SHACLObjectSet::addContext(std::string const &prefix, std::string const &expanded) {
  mContext[prefix] = expanded;
}

std::string SHACLObjectSet::expandIRI(std::string const &iri) const {
  for (auto const &[prefix, expanded] : mContext) {
    if (iri == prefix) {
    ...
    auto p = prefix + ":";
    if (iri.rfind(p, 0) == 0) {
      return expandIRI(expanded + iri.substr(p.length()));
    }
  }
  return iri;
}

std::string SHACLObjectSet::compactIRI(std::string const &iri) const {
  for (auto const &[prefix, expanded] : mContext) {
    if (iri == expanded) {
      return compactIRI(prefix);
    }
    if (iri.rfind(expanded, 0) == 0) {
      return compactIRI(prefix + ":" + iri.substr(expanded.length()));
    }
  }
  return iri;
}

SHACLObjectSet::Context const &SHACLObjectSet::context() const { return mContext; }
