...
bool SHACLObject::IDPropConfig::validateValue(std::string const &val, ErrorHandler &errorHandler,
  ...
  case NodeKind::BlankNode:
    if (!isBlankNode(val)) {
      errorHandler.handleValidationError("ID must be a blank node", path);
      valid = false;
    }
    break;
  case NodeKind::IRI:
  ...
  default:
    if (!isBlankNode(val) && !isIRI(val)) {
      errorHandler.handleValidationError("ID must be a blank node or IRI", path);
      valid = false;
    }
    break;
  }
...
void SHACLObject::IDPropConfig::encode(ValueEncoder &encoder, std::string const &value,
                                       ErrorHandler &errorHandler, ObjectPath &path) const {
  throw std::runtime_error("ID should not be directly encoded");
}

std::optional<std::string> SHACLObject::IDPropConfig::decode(ValueDecoder &decoder,
                                                             ErrorHandler &errorHandler,
                                                             ObjectPath &path) const {
  auto value = StringProperty::decode(decoder, errorHandler, path);
  if (!value) {
    return value;
  }
  return decoder.getState().expandIRI(value.value());
}
...
SHACLObject::SHACLObject(TypeBase const *type, TypeIRIs const &typeIRIs)
    ...
    // owner is nullptr
    : _id(nullptr, type->getIDAlias().value_or("@id"), {}, type->getNodeKind() == NodeKind::IRI,
          this),
      mType(type), mTypeIRIs(typeIRIs) {}
...
void SHACLObject::encode(ValueEncoder &encoder, ErrorHandler &errorHandler,
  ...
  auto const objId = state.compactIRI(state.getObjectId(*this));
  ...
  encoder.writeObject(*this, getCompactTypeIRI().value_or(state.compactIRI(getTypeIRI())), objId,
                      _id.isSet() || state.objectIsRefed(*this), [&](ObjectEncoder &objectEncoder) {
                        encodeProperties(objectEncoder, errorHandler, path);
                      });
}

void SHACLObject::encodeProperties(ObjectEncoder &encoder, ErrorHandler &errorHandler,
                                   ObjectPath &path) const {
  for (auto const *p : mProperties) {
    if (p->elide()) {
      continue;
    ...
    path.pushProperty(*p, [&] {
      encoder.writeProperty(p->getIRI(), p->getCompactIRI(), [&](ValueEncoder &valueEncoder) {
        p->encode(valueEncoder, errorHandler, path);
      });
    });
  }
}

void SHACLObject::handleUnknownDecodeProperty(std::string const &name, ValueDecoder &decoder,
...
std::shared_ptr<SHACLObject> SHACLObject::decode(ObjectDecoder &decoder, TypeBase const *type,
                                                 ErrorHandler &errorHandler, ObjectPath &path) {
  auto t = decoder.readType();
  if (!t) {
    errorHandler.handleDeserializeError("Unable to decode object type"s, path);
    return nullptr;
  }

  auto const &obj_type_iri = decoder.getState().expandIRI(t.value());
  ...
  if (iri_type) {
    if (type && !iri_type->isSubClassOf(type)) {
      errorHandler.handleDeserializeError("Type "s + obj_type_iri + " is not valid where "s +
    ...
    for (auto const &[iri, extType] : extTypes) {
      ...
      obj = SHACLObject::decode(decoder, extType, NoopErrorHandler::handler, path);
      if (obj) {
        return obj;
      }
    }
  ...
  if (!obj) {
    errorHandler.handleDeserializeError("Unable to create object of type "s + obj_type_iri, path);
    return obj;
  }

  decoder.readProperties([&](std::string const &name, ValueDecoder &value) {
    if (obj->_id.matchesIRI(name)) {
      path.pushProperty(obj->_id, [&] { obj->_id.decode(value, errorHandler, path); });
      return;
    }

    for (auto *p : obj->mProperties) {
      if (p->matchesIRI(name)) {
        path.pushProperty(*p, [&] { p->decode(value, errorHandler, path); });
        return;
      }
    }
...
std::unordered_map<std::string, TypeBase const *> &SHACLObject::getIRITypeRegistry() {
  static std::unordered_map<std::string, TypeBase const *> IRITypeRegistry;
  return IRITypeRegistry;
}

std::unordered_map<std::string, TypeBase const *> &SHACLObject::getCompactTypeRegistry() {
