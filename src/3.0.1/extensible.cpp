...
std::vector<Variant> &SHACLExtensibleObjectBase::getExtProperty(std::string const &name) {
  auto r = mExtendedProperties.insert({name, std::vector<Variant>()});
  return r.first->second;
}

void SHACLExtensibleObjectBase::encodeExtensibleProperties(ObjectEncoder &encoder,
  ...
  for (auto const &p : mExtendedProperties) {
    ...
    auto const iri = encoder.getState().compactIRI(p.first);
    path.pushMember(iri, [&] {
      encoder.writeProperty(iri, {}, [&](ValueEncoder &prop_encoder) {
            ...
            list_encoder.writeItem([&](ValueEncoder &item_encoder) {
              std::visit(
                  [&](auto &&value) {
                    ...
                    } else if constexpr (std::is_same_v<T, Ref<SHACLObject>>) {
                      if (value.isObj()) {
                        value.obj()->encode(item_encoder, errorHandler, path);
                      } else {
                        item_encoder.writeIRI(state.compactIRI(value.iri()), {});
                      }

                    } else {
                      static_assert(always_false_v<T>, "Not all possible "
                                                       "types handled");
                    }
                  },
                  variant);
...
void SHACLExtensibleObjectBase::decodeExtensibleProperty(std::string const &name,
                                                         ValueDecoder &decoder,
                                                         ErrorHandler &errorHandler,
                                                         ObjectPath &path) {
  auto &p = getExtProperty(decoder.getState().expandIRI(name));

  path.pushMember(name, [&] {
    if (decoder.getType() == ValueDecoder::Type::List) {
      decoder.readList([&](ListDecoder &list_decoder) {
        path.foreachListDecoder(list_decoder, [&](ValueDecoder &item_decoder) {
          decodeExtPropertyValue(p, item_decoder, errorHandler, path);
        });
      });
    } else {
...
void SHACLExtensibleObjectBase::decodeExtPropertyValue(std::vector<Variant> &prop,
                                                       ValueDecoder &value,
                                                       ErrorHandler &errorHandler,
                                                       ObjectPath &path) {
  ...
  case ValueDecoder::Type::Object: {
    ...
    bool ret = value.readObject([&](ObjectDecoder &obj_decoder) {
      obj = SHACLObject::decode(obj_decoder, nullptr, errorHandler, path);
    });
