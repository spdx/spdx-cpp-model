...
/* */
#ifndef _SHACL2CODE_SPDX_3_0_1_REFPROPERTY_HPP
#define _SHACL2CODE_SPDX_3_0_1_REFPROPERTY_HPP
...
public:
   ...
   */
  static std::shared_ptr<SHACLObject> decodeSHACLObject(ObjectDecoder &decoder,
                                                        TypeBase const *type,
   ...
   */
  static void walkSHACLObject(SHACLObject const *object, WalkCallback const &callback,
                              ObjectPath &path);
};
 ...
 *              SHACLObject
 */
template <class C> class RefProperty : public Property<Ref<C>>, private RefPropertyHelper {
public:
  /// Constructor
  RefProperty(std::optional<std::regex> &&regex = {}, PropertyContext::Context &&context = {})
  ...
  bool validateValue(Ref<C> const &value, ErrorHandler &errorHandler,
    ...
    if (value.isIRI()) {
      if (!isIRI(value.iri()) && !isBlankNode(value.iri())) {
        errorHandler.handleValidationError(std::string("Value '") + value.iri() +
      ...
      if (named && !named->getType()->isSubClassOf(&C::Type)) {
        errorHandler.handleValidationError(std::string("Named individual '") + value.iri() +
  ...
  void encode(ValueEncoder &encoder, Ref<C> const &value, ErrorHandler &errorHandler,
              ObjectPath &path) const override {
    if (value.isObj()) {
      value.obj()->encode(encoder, errorHandler, path);
    } else {
      encoder.writeIRI(value.iri(),
                       this->compactValue(value.iri(), encoder.getState().compactIRI(value.iri())));
    }
  }

  /// @copydoc ::SPDX::v3_0_1::Property::decode()
  std::optional<Ref<C>> decode(ValueDecoder &decoder, ErrorHandler &errorHandler,
                               ObjectPath &path) const override {
    auto iri = decoder.readIRI();
    if (iri) {
      return Ref<C>(
          this->expandValue(iri.value()).value_or(decoder.getState().expandIRI(iri.value())));
    }
  ...
  /// @copydoc ::SPDX::v3_0_1::Property::link()
  void link(LinkState &state, Ref<C> &value) const override { linkRef(state, value); }
...
/* */
/* */
#endif // _SHACL2CODE_SPDX_3_0_1_REFPROPERTY_HPP
/* */
