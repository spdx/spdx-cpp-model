...
public:
  /// Constructor
  explicit Finally(std::function<void(void)> &&func) : mFunc(std::move(func)) {}
...
std::string ObjectPath::getPathString() const {
  ...
  std::copy(mPath.begin(), mPath.end(), std::ostream_iterator<std::string>(s));
...
void ObjectPath::pushMember(std::string const &name, ObjectPath::func const &f) {
  pushPath("."s + name, f);
}

void ObjectPath::pushProperty(PropertyValueCommon const &p, ObjectPath::func const &f) {
  pushMember(p.getCompactIRI().value_or(p.getIRI()), f);
}

void ObjectPath::foreachListDecoder(ListDecoder &decoder,
