...
/* */
#ifndef _SHACL2CODE_SPDX_3_0_1_REF_HPP
#define _SHACL2CODE_SPDX_3_0_1_REF_HPP
 ...
 * ::SPDX::v3_0_1::SHACLObject::_id
 *
 * @see ::SPDX::v3_0_1::SHACLObject::_id
 *
...
public:
   ...
   */
  template <class T, class = std::enable_if_t<std::is_base_of<SHACLObject, T>::value &&
                                              std::is_convertible<T *, C *>::value>>
  Ref(Ref<T> const &other) : mValue(std::move(other.template asTypeVariant<C>())) {}
   ...
   */
  bool isObj() const { return std::holds_alternative<std::shared_ptr<C>>(mValue); }
   ...
   * object instance
   */
  std::shared_ptr<C> const &obj() const { return std::get<std::shared_ptr<C>>(mValue); }
  ...
  template <class T, class = std::enable_if_t<std::is_base_of<SHACLObject, T>::value &&
                                              (std::is_convertible<T *, C *>::value ||
                                               std::is_convertible<C *, T *>::value)>>
  static Ref<C> fromType(std::variant<std::string, std::shared_ptr<T>> const &value) {
  ...
  template <class T, class = std::enable_if_t<std::is_base_of<SHACLObject, T>::value &&
                                              (std::is_convertible<T *, C *>::value ||
                                               std::is_convertible<C *, T *>::value)>>
  bool isType() const {
    if constexpr (!std::is_base_of<C, T>::value && !std::is_base_of<T, C>::value) {
      return false;
    }
 ...
 */
template <class C> Ref<C> make_obj() { return Ref<C>(std::make_shared<C>()); }
...
/* */
/* */
#endif // _SHACL2CODE_SPDX_3_0_1_REF_HPP
/* */
