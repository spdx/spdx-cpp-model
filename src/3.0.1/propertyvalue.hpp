...
/* */
#ifndef _SHACL2CODE_SPDX_3_0_1_PROPERTYVALUE_HPP
#define _SHACL2CODE_SPDX_3_0_1_PROPERTYVALUE_HPP
...
public:
   ...
   */
  virtual void decode(ValueDecoder &decoder, ErrorHandler &errorHandler, ObjectPath &path) = 0;
 ...
 */
template <typename C, class PROP> class PropertyValue : public PropertyValueCommon {
...
public:
  ...
  PropertyValue(SHACLObject *owner, std::string const &iri,
                std::optional<std::string> const &compact = {}, bool mandatory = false,
                Args &&...args)
      : PropertyValueCommon(owner, iri, compact), mConfig(std::forward<Args>(args)...),
  ...
  /// @copydoc ::SPDX::v3_0_1::PropertyValueCommon::encode()
  void encode(ValueEncoder &encoder, ErrorHandler &errorHandler, ObjectPath &path) const override {
    mConfig.encode(encoder, get(), errorHandler, path);
  }

  /// @copydoc ::SPDX::v3_0_1::PropertyValueCommon::decode()
  void decode(ValueDecoder &decoder, ErrorHandler &errorHandler, ObjectPath &path) override {
  ...
  /// @copydoc ::SPDX::v3_0_1::PropertyValueCommon::elide()
  bool elide() const override { return !isSet(); }

  /// @copydoc ::SPDX::v3_0_1::PropertyValueCommon::link()
  ...
  /// @copydoc ::SPDX::v3_0_1::PropertyValueCommon::walk()
  ...
  /// @copydoc ::SPDX::v3_0_1::PropertyValueCommon::check()
  bool check(ErrorHandler &errorHandler, ObjectPath &path) const override {
    bool valid = true;

    if (mMandatory && !isSet()) {
      errorHandler.handleValidationError(getIRI() + std::string(" is required"), path);
      valid = false;
    }
   ...
   */
  bool validateValue(C const &v, ErrorHandler &errorHandler, ObjectPath &path) {
 ...
 */
template <class C> class RefPropertyValue : public PropertyValue<Ref<C>, RefProperty<C>> {
public:
  /// Constructor
  template <typename... Args>
   ...
   * @see Ref<T>::asTypeRef()
   */
  template <class T> Ref<T> asTypeRef() const {
    if (!this->isSet()) {
      throw std::bad_cast();
    }
 ...
 * ::SPDX::v3_0_1::ListPropertyValue::insert(),
 * ::SPDX::v3_0_1::ListPropertyValue::push_back(),
 * ::SPDX::v3_0_1::ListPropertyValue::add()
 * will be validated at the time of insertion. However, if an item is modified
 * via an iterator, ::SPDX::v3_0_1::ListPropertyValue::at() or
 * ::SPDX::v3_0_1::ListPropertyValue::operator[],
 * Its new value will not be validated when it is changed. Validation can be
 * performed manually using ::SPDX::v3_0_1::ListPropertyValue::validateValue().
 ...
 *              Must derive from Property<C>
 */
template <typename C, class PROP> class ListPropertyValue : public PropertyValueCommon {
  static_assert(std::is_base_of<Property<C>, PROP>::value,
                "template class C must inherit from Property<C>");
...
public:
  ...
  ListPropertyValue(SHACLObject *owner, std::string const &iri,
                    ...
                    std::optional<std::size_t> const &maxCount = {}, Args &&...args)
      : PropertyValueCommon(owner, iri, compact), mConfig(std::forward<Args>(args)...),
  ...
  ListPropertyValue &operator=(std::initializer_list<C> ilist) {
    ObjectPath path;
    path.foreachItem(ilist,
                     [&](C const &v) { validateValue(v, DefaultErrorHandler::handler, path); });
   ...
   */
  template <typename ITER> auto erase(ITER pos) { return mValue.erase(pos); }
   ...
   */
  template <typename ITER> auto erase(ITER first, ITER last) { return mValue.erase(first, last); }
   ...
   * @returns true if the value is valid for the property, and false if not
   */
  bool validateValue(C const &v, ErrorHandler &errorHandler, ObjectPath &path) const {
    return mConfig.validateValue(v, errorHandler, path);
  }
   ...
   * @returns true if the value is valid for the property, and false if not
   */
  bool validateValue(const_iterator position, ErrorHandler &errorHandler, ObjectPath &path) const {
   ...
   */
  bool validateValue(const_iterator first, const_iterator last, ErrorHandler &errorHandler,
  ...
  void encode(ValueEncoder &encoder, ErrorHandler &errorHandler, ObjectPath &path) const override {
    encoder.writeList([&](ListEncoder &listEncoder) {
      path.foreachItem(*this, [&](C const &v) {
        listEncoder.writeItem([&](ValueEncoder &e) { mConfig.encode(e, v, errorHandler, path); });
      });
    });
  }
  ...
  void decode(ValueDecoder &decoder, ErrorHandler &errorHandler, ObjectPath &path) override {
    ...
    auto ret = decoder.readList([&](ListDecoder &list_decoder) {
      path.foreachListDecoder(list_decoder, [&](ValueDecoder &value_decoder) {
  ...
  /// @copydoc ::SPDX::v3_0_1::PropertyValueCommon::walk()
  void walk(WalkCallback const &callback, ObjectPath &path) const override {
    path.foreachItem(*this, [&](C const &v) { mConfig.walk(callback, std::cref<C>(v), path); });
  }
  ...
  bool check(ErrorHandler &errorHandler, ObjectPath &path) const override {
    ...
    if (mMinCount.has_value()) {
      if (size() < mMinCount.value()) {
        errorHandler.handleValidationError(getIRI() + std::string(" requires at least ") +
    ...
    if (mMaxCount.has_value()) {
      if (size() > mMaxCount.value()) {
        errorHandler.handleValidationError(getIRI() + std::string(" cannot have more than ") +
 ...
 * @tparam C    The type of the value. Must derive from SHACLObject
 */
template <class C> class RefListPropertyValue : public ListPropertyValue<Ref<C>, RefProperty<C>> {
public:
  /// Constructor
  template <typename... Args>
  RefListPropertyValue(SHACLObject *owner, std::string const &iri,
                       ...
                       std::optional<std::size_t> const &minCount = {},
                       std::optional<std::size_t> const &maxCount = {}, Args &&...args)
      : ListPropertyValue<Ref<C>, RefProperty<C>>(owner, iri, compact, minCount, maxCount,
                                                  std::forward<Args>(args)...) {}
...
/* */
/* */
#endif // _SHACL2CODE_SPDX_3_0_1_PROPERTYVALUE_HPP
/* */
